...
morphgames.io`;
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent 
        className="max-w-md max-h-[90vh] overflow-y-auto bg-white border-[hsl(var(--grid-card-border))]"
          ...
          <div className="max-h-36 overflow-y-auto space-y-2 p-3 bg-[hsl(var(--grid-pill-bg))] rounded-lg">
            ...
            <div className="flex flex-wrap gap-1.5">
              {submittedWords.map((w, i) => (
                <div
                  key={i}
                  className="px-2 py-0.5 bg-white text-[hsl(var(--grid-text-secondary))] rounded text-xs font-medium border border-[hsl(var(--grid-card-border))]"
                >
                  {w.word}
                </div>
              ))}
